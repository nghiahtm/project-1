...
if (isset($_GET['id_technology'])) {
    ...
    $dataProducts = responseData(
        'SELECT products.id_product,products.name_product,products.link_image,products.price FROM products
...
if (isset($_POST['decrement'])) {
    if (!isset($_GET['id'])) {
        $dataProducts = responseData('SELECT products.id_product,products.name_product,products.link_image,products.price FROM products ORDER BY price DESC');
    } else {
        $dataProducts = responseData('SELECT products.id_product,products.name_product,products.link_image,products.price FROM products
     LEFT JOIN configurations c on products.id_config = c.id_configurations
    Left Join main_product mp on c.id_main_product = mp.id_main_product
    Left join categories c2 on c2.id_category = mp.id_category
...
if (isset($_POST['increment'])) {
    if (!isset($_GET['id'])) {
        $dataProducts = responseData('SELECT products.id_product,products.name_product,products.link_image,products.price FROM products ORDER BY price');
    } else {
        $dataProducts = responseData('SELECT products.id_product,products.name_product,products.link_image,products.price FROM products
     LEFT JOIN configurations c on products.id_config = c.id_configurations
    Left Join main_product mp on c.id_main_product = mp.id_main_product
